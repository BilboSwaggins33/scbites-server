...
const {
  LegendConstantsFlipped,
  LocationsFlipped,
  Locations,
} = require("../constants/legend");
...
async function grabUserData() {
  try {
    ...
    for await (let doc of users) {
      ...
      doc["keywords"] = await keywordsCollection
        .find({ owner_id: uid })
        .toArray();
...
async function scrape(location) {
  try {
    ...
    const { data } = await axios.get(
      `${MENU_BASE_URL}${location}&menu_date=${currentDate}`,
    );
    ...
    menuElements.each((idx, el) => {
      ...
      const mealTypeText = mealTypeChild
        .first()
        .contents()
        .filter(function () {
      ...
      menuItems.push({
        name: item
          ...
          .text(),
        tags: tags,
        location: LocationsFlipped[location],
        meal: mealTypeText,
      });
    });
    return menuItems;
...
async function sendEmails() {
  const dronepic = fs
  .readFileSync(`${process.cwd()}/public/images/droneusc.jpg`)
  .toString("base64");

  let userData = await grabUserData();
  ...
  for (let user of userData) {
    if (user.isActive) {
      ...
      const emailData = [...keywordParkside, ...keywordVillage, ...keywordEvk];

      function isDuplicate(entry, arr) {
        return arr.some(
          (x) => entry.name === x.name && entry.location === x.location,
        );
      }
      ...
      // no email if user has no items
      if (uniqueData.length == 0) {
        continue;
      }

      const html = await ejs.renderFile(
        path.resolve(__dirname, "../views/template2.ejs"),
        {
          emailData: uniqueData,
          dronepic: dronepic
        },
      );
        ...
        transporter.sendMail(
          {
            ...
            to: user.email,
            subject: `You have dishes at ${uniqueData[0].location}` + (uniqueData.length > 1 ? ' and more!' : ''),
            attachments: [
              {
                filename: "menu.pdf",
                content: pdf,
...
module.exports = { sendEmails, scrape };
